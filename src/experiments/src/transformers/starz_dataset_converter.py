...
class StarzDatasetConverter(BaseTransformer):
    ...
    def fit(self, df: pd.DataFrame) -> 'StarzDatasetConverter':
        """
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_columns = {'match_id', 'radiant_team', 'dire_team', 'radiant_win', 'actualRank'} \
                                                        .union({f'dire_{i}' for i in range(1, 6)}) \
                                                        .union({f'radiant_{i}' for i in range(1, 6)})
