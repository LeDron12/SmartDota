...
# Add exp2 directory to path to import shared modules
exp2_path = Path(__file__).parent.parent / "exp2"
sys.path.append(str(exp2_path))
...
def main(base_path: Path):
    ...
    models_path = base_path / "models_metrics"
    models_path.mkdir(parents=True, exist_ok=True)
    input_sample_path = base_path / "input_sample"
    input_sample_path.mkdir(parents=True, exist_ok=True)
    
    logger.info("Starting draft win prediction experiment (Hero ID features)...")
    ...
    plot_metrics_over_iterations(trainer, models_path)
    plot_threshold_metrics(trainer, models_path)
...
if __name__ == "__main__":
    base_path = Path("/Users/ankamenskiy/SmartDota/src/experiments/cursor_exps/exp1")
    main(base_path) 